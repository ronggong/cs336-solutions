...
def run_linear(
    ...
    torch.nn.Module.load_state_dict(linear, state_dict)
...
def run_embedding(
    ...
    torch.nn.Module.load_state_dict(embedding, state_dict)
...
def run_swiglu(
    ...
    from cs336_basics.positionwise_feedforward import PWFF
    ff = PWFF(d_model)
    ff.d_ff = d_ff
...
def run_multihead_self_attention(
    ...
    """
    from cs336_basics.mhsa import MHSA
    msha = MHSA(d_model, num_heads)
    ...
    msha.load_state_dict(state_dict)
    return msha(in_features)
...
def run_transformer_block(
    ...
    from cs336_basics.rope import RotaryPositionEmbedding
    rope = RotaryPositionEmbedding(theta, d_model / num_heads, max_seq_len)
    trans = TransformerBlock(d_model, num_heads, d_ff, rope)
    trans.load_state_dict(weights)
    tokens_position = torch.linspace(1, in_features.size(-2), steps=in_features.size(-2), dtype=torch.int)
    return trans(in_features, tokens_position)
...
def run_transformer_lm(
    ...
    lm = TransformerLM(vocab_size, d_model, num_heads, d_ff, context_length, rope_theta, num_layers)
    lm.load_state_dict(weights)
    tokens_position = torch.linspace(1, in_indices.size(-1), steps=in_indices.size(-1), dtype=torch.int)
    return lm(in_indices, tokens_position)
...
def run_rmsnorm(
    ...
    torch.nn.Module.load_state_dict(rmsnorm, state_dict)
